...
const Analytics = () => {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      ...
      console.log('Attempted session ID:', location.state?.sessionId);
  ...
  }, [isLive, location.state?.sessionId, fetchAnalytics]);
  ...
  // Safeguards for optional/missing fields from API
  const fillerWords = analyticsData.fillerWords || { total: 0, breakdown: [], realtime_count: 0 };
  const scores = {
    ...
    // Derive communication if server doesn't provide it
    communication: analyticsData.scores?.communication ?? Math.round(((analyticsData.scores?.confidence ?? 0) + (analyticsData.scores?.clarity ?? 0)) / 2)
  };
  const feedback = {
    strengths: analyticsData.feedback?.strengths ?? [],
    improvements: analyticsData.feedback?.improvements ?? [],
    // Some server builds may not include recommendations; fall back to top improvements
    recommendations: analyticsData.feedback?.recommendations ?? (analyticsData.feedback?.improvements ?? [])
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-white mb-2">{fillerWords.total}</div>
              <div className="text-purple-300">
                Filler Words
                {fillerWords.realtime_count > 0 && (
                  <span className="text-yellow-400 text-sm ml-1">
                    (+{fillerWords.realtime_count} live)
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={[
                ...
                { name: 'Technical', score: scores.technical_accuracy },
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  dataKey="count"
                >
                  {(fillerWords.breakdown || []).map((entry, index) => (
